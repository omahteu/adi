async function carregar_equipamentos_substituicao(valor) {
    if (valor.length >= 3) {
        document.getElementById('resultado_pesquisa_equipamento').style.display = "inline";
        const dados = await fetch('equ_sub.php?nome=' + valor);
        ...
        if (resposta['erro']) {
            ...
            for (i = 0; i < resposta['dados'].length; i++) {
                htmlE += "<li class='list-group-item list-group-item-action' onclick='get_id_equipamentos_substituicao(" + resposta['dados'][i].id + "," + JSON.stringify(resposta['dados'][i].nome) + ")'>" + resposta['dados'][i].nome + "</li>";
            }
        }
        htmlE += "</ul>";
...
function get_id_equipamentos_substituicao(id_equipamento, nome) {
