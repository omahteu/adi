async function carregar_usuarios_substituicao(valor) {
    if (valor.length >= 3) {
        document.getElementById('resultado_pesquisa').style.display = "inline";
        const dados = await fetch('usu_sub.php?nome=' + valor);
        ...
        if (resposta['erro']) {
            ...
            for (i = 0; i < resposta['dados'].length; i++) {
                htmlU += "<li class='list-group-item list-group-item-action' onclick='get_id_usuario_substituicao(" + resposta['dados'][i].id + "," + JSON.stringify(resposta['dados'][i].nome_razao_social) + ")'>" + resposta['dados'][i].nome_razao_social + "</li>";
            }
        }
        htmlU += "</ul>";
...
function get_id_usuario_substituicao(id_usuario, nome) {
